...
profile.set_preference("network.dnsCacheExpiration",False) #禁止使用浏览器dns缓存
...
driver = webdriver.Firefox(profile) # 火狐浏览器驱动，该浏览器较为干净
...
def main():
    ...
    for i in urls:
        # i = 'hit.edu.cn'
        np = Process(target=extract_capture,args=[i,20])
